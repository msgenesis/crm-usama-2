...
class CallBack extends Component {
        ...
        if (cc === " " && this.state.data[this.state.modalIndex].cardDetail === "") {
            let deal = {
                ...
                status: {
                    ...
                    statusCloser: this.state.oldSSNList.indexOf(ssn) !== - 1 || this.state.oldPhoneList.indexOf(phone) !== -1 ? "Dublicated" : this.state.data[this.state.modalIndex].status.statusCloser,
            ...
            console.log("Testingg=====>>>", duePayDate === " " || duePayDate === "" ? this.state.data[this.state.modalIndex].cardDetail[cardIndex].duePayDate : duePayDate)
            ...
            let deal = {
                ...
                status: {
                    ...
                    transferAgentID: this.state.data[this.state.modalIndex].status.transferAgentID,
                    transferAgentName: this.state.data[this.state.modalIndex].status.transferAgentName,
                    statusCloser: this.state.oldSSNList.indexOf(ssn) !== - 1 || this.state.oldCardList.indexOf(this.state.data[this.state.modalIndex].cardDetail[cardIndex].cc) !== -1 || this.state.oldPhoneList.indexOf(phone) !== -1 ? "Dublicated" : this.state.data[this.state.modalIndex].status.statusCloser,
                    transferDate: this.state.data[this.state.modalIndex].status.transferDate === undefined ? "" : this.state.data[this.state.modalIndex].status.transferDate
                },
                otherDetail: {
                    SecurityWord: SecurityWord === " " ? this.state.data[this.state.modalIndex].otherDetail.SecurityWord : SecurityWord,
    ...
    //                 if (data.val()[uid][key].status.statusCloser === "Call Back") {
    ...
    saleTransfer(admin, i) {
        ...
        //             console.log("Checkingg=>>", Admin.id)
                                                ...
                                                <tr key={i}>
                                                    ...
                                                    <td>
                                                        {v.Status === "Dublicated" ?
                                                            ...
                                                            :
                                                            <Button size="sm" color={v.Status === "Transfer" ? 'outline-dark' : 'outline-primary'} onClick={this.showModal.bind(this, i)} disabled={v.Status === "Transfer" ? true : v.Status === "Dublicated" ? true : false}><i className={v.Status === "Transfer" ? "fa fa-exclamation-circle" : "fa fa-search"}></i> {v.Status === "Transfer" ? "Disabled" : "View"}</Button>
                                                        }
                                                    </td>
                                                    <td>
                                                        {/* {v.uid === user.uid || v.Status === "Transfer" ? */}
                                                        { v.Status === "Transfer" ?
...
export default connect(mapStateToProp, mapDispatchToProp)(CallBack);
